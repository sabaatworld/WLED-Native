...
class AutoSaveHostUsermod : public HostUsermod {
public:
  const char* name() const override { return "Autosave"; }
  uint16_t getId() const override { return WLED_HOST_USERMOD_ID_AUTO_SAVE; }

  void setup(HostUsermodContext& context) override {
    initialized_ = true;
    if (enabled_ && applyAutoSaveOnBoot_) context.applyPreset(autoSavePreset_);
    knownState_ = context.getState();
  }

  void loop(HostUsermodContext& context) override {
    if (!initialized_ || !enabled_ || autoSaveAfterSec_ == 0) return;

    const HostRuntimeState currentState = context.getState();
    if (autoSaveIgnorePresets_ && currentState.preset > 0) {
      knownState_ = currentState;
      pendingSaveAtMs_ = 0;
      return;
    }
    ...
    if (stateChanged(currentState)) {
      knownState_ = currentState;
      pendingSaveAtMs_ = context.millis() + static_cast<uint64_t>(autoSaveAfterSec_) * 1000U;
    }

    if (pendingSaveAtMs_ > 0 && context.millis() >= pendingSaveAtMs_) {
      pendingSaveAtMs_ = 0;
      context.savePreset(autoSavePreset_, "Autosave");
    }
  }

  void addToJsonState(HostUsermodContext&, JsonObject& root) override {
    JsonObject usermod = root[name()].is<JsonObject>() ? root[name()].as<JsonObject>() : root.createNestedObject(name());
    usermod["enabled"] = enabled_;
  }

  void addToJsonInfo(HostUsermodContext&, JsonObject& root) override {
    JsonObject user = root["u"].is<JsonObject>() ? root["u"].as<JsonObject>() : root.createNestedObject("u");
    JsonArray info = user.createNestedArray(name());
    info.add(enabled_ ? "Enabled" : "Disabled");
    info.add(autoSavePreset_);
  }

  void readFromJsonState(HostUsermodContext&, JsonObjectConst root) override {
    JsonObjectConst usermod = root[name()].as<JsonObjectConst>();
    if (usermod.isNull()) return;
    if (usermod["enabled"].is<bool>()) enabled_ = usermod["enabled"].as<bool>();
  }

  void addToConfig(JsonObject& root) override {
    JsonObject top = root.createNestedObject(name());
    top["enabled"] = enabled_;
    top["autoSaveAfterSec"] = autoSaveAfterSec_;
    top["autoSavePreset"] = autoSavePreset_;
    top["autoSaveApplyOnBoot"] = applyAutoSaveOnBoot_;
    top["autoSaveIgnorePresets"] = autoSaveIgnorePresets_;
  }

  bool readFromConfig(JsonObjectConst root) override {
    JsonObjectConst top = root[name()].as<JsonObjectConst>();
    if (top.isNull()) return false;

    enabled_ = top["enabled"] | enabled_;
    autoSaveAfterSec_ = static_cast<uint16_t>(top["autoSaveAfterSec"] | autoSaveAfterSec_);
    autoSaveAfterSec_ = std::clamp<uint16_t>(autoSaveAfterSec_, 1, 3600);
    autoSavePreset_ = static_cast<uint8_t>(top["autoSavePreset"] | autoSavePreset_);
    autoSavePreset_ = std::clamp<uint8_t>(autoSavePreset_, 100, 250);
    applyAutoSaveOnBoot_ = top["autoSaveApplyOnBoot"] | applyAutoSaveOnBoot_;
    autoSaveIgnorePresets_ = top["autoSaveIgnorePresets"] | autoSaveIgnorePresets_;
    return true;
  }

private:
  bool stateChanged(const HostRuntimeState& state) const {
    return knownState_.bri != state.bri ||
           knownState_.segmentBri != state.segmentBri ||
           knownState_.effect != state.effect ||
           knownState_.speed != state.speed ||
           knownState_.intensity != state.intensity ||
           knownState_.palette != state.palette ||
           knownState_.colors != state.colors;
  }

  bool initialized_ = false;
  bool enabled_ = true;
  uint16_t autoSaveAfterSec_ = 15;
  uint8_t autoSavePreset_ = 250;
  bool applyAutoSaveOnBoot_ = false;
  bool autoSaveIgnorePresets_ = true;
  uint64_t pendingSaveAtMs_ = 0;
  HostRuntimeState knownState_;
};
// AI: end
...
std::unique_ptr<HostUsermod> createAutoSaveHostUsermod() {
  return std::make_unique<AutoSaveHostUsermod>();
}
