...
Before finishing any native-port task:
...
Nothing in the native migration should be undocumented. If a task changes direction,
update the plan first or in the same patch so later work has a reliable handoff.

### Repo-Level Codex Hooks

This repo defines project-local Codex hooks in `.codex/hooks.json`, with hook
scripts in `.codex/hooks/`. Codex discovers repo hooks from `<repo>/.codex/hooks.json`
or inline `<repo>/.codex/config.toml`; keep this repo on the `hooks.json` form only.

- Review and trust changed hooks with `/hooks` before expecting them to run.
...
- Hooks are guardrails, not a substitute for the task process above. If a hook
  detects missing migration documentation, update the plan instead of bypassing
  the hook.
